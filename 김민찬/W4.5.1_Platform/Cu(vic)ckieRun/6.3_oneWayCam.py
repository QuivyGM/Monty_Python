"""
Platformer Game with One-Way Camera Movement, Infinite Ground, Random Crates, and Controller Support
"""
import arcade
import pygame
import random
...
class MyGame(arcade.Window):
    ...
    def __init__(self):
        ...
        # Camera
        self.camera = None
        self.camera_max_x = 0  # Max position the camera can scroll right
    ...
    def on_update(self, delta_time):
        ...
        # Center the camera to follow the player, moving only right
    ...
    def center_camera_to_player(self, delta_time):
        """Smoothly center the camera on the player, but only allow forward movement."""
        screen_center_x = self.player_sprite.center_x - (self.camera.viewport_width / 2)
        screen_center_y = self.player_sprite.center_y - (self.camera.viewport_height / 2)

        # Ensure the camera doesn't scroll back left
        screen_center_x = max(screen_center_x, self.camera_max_x)
        screen_center_y = max(screen_center_y, 0)

        # Update the max x position the camera has reached
        self.camera_max_x = max(self.camera_max_x, screen_center_x)
